...
class CBXPollAdmin
{
	...
	function on_poll_delete_vote_delete($poll_id)
    {
	    ...
	    if (! empty($poll_votes)) {
		    foreach ($poll_votes as $log_info) {
			    ...
			    $delete_status = $wpdb->query($wpdb->prepare("DELETE FROM {$poll_vote_table} WHERE id = %d", $id)); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    ...
    public function get_setting_fields()
    {
        ...
        $fields = [
            ...
            /*'cbxpoll_email_setting'   => apply_filters( 'cbxpoll_global_email_fields2', [
				'email_setting_heading' => [
            ...
            'cbxpoll_email_tpl'       => apply_filters('cbxpoll_global_email_fields', [
                ...
                'headerimage'            => [
                    'name'    => 'headerimage',
                    'label'   => esc_html__('Header Image', 'cbxpoll'),
                    //'desc'    => esc_html__( 'Url To email you want to show as email header.Upload Image by media uploader.','cbxpoll' ),
                    'type'    => 'file',
                    'default' => '',
                ],
                'footertext'             => [
            ...
            'cbxpoll_tools'           => apply_filters('cbxpoll_global_tools_fields', [
                ...
                'delete_global_config' => [
                    ...
                    'desc'    => '<p>'.esc_html__('Delete Global Config data and custom table created by this plugin on uninstall.',
                            'cbxpoll').' '.wp_kses_post(__('Details table information is <a href="#cbxpoll_plg_gfig_info">here</a>',
                            'cbxpoll')).'</p>'.'<p><strong>'.esc_html__('Please note that this process can not be undone and it is recommended to keep full database backup before doing this.',
                            'cbxpoll').'</strong></p>',
                    'type'    => 'radio',
                    'options' => [
...
<a class="button button-primary" onclick="return confirm(\'%s\')" href="%s">Reset Data</a>', 'cbxpoll' ),
        ...
        if ($post_id > 0):
            ...
            if ( ! $is_voted && sizeof($poll_answers) == 0) {
                ...
                foreach ($default_answers_titles as $index => $answers_title) {
                    //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
                    echo PollHelper::cbxpoll_answer_field_template(absint($index) + $new_index, esc_html($default_answers_titles[$index]), esc_html($default_answers_colors[$index]), absint($is_voted), $answers_extra, absint($post_id));
                }
            ...
            <div class="add-cbx-poll-answer-wrap" data-busy="0" data-postid="<?php echo absint($post_id); ?>">
                <a data-type="default" id="add-cbx-poll-answer-default"
                   class="button primary icon icon-inline add-cbx-poll-answer add-cbx-poll-answer-default add-cbx-poll-answer-<?php echo absint($post_id); ?>">
                    <i class="cbx-icon"><?php echo $plus_svg; //phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?></i>
     ...
     */
    public function plugin_full_reset()
    {
        //phpcs:ignore WordPress.Security.NonceVerification.Recommended
           ...
           /* $table_names  = PollHelper::getAllDBTablesList();
	        ...
	        if ( ! empty( $table_names ) && is_array( $table_names ) ) {
		        foreach ( $table_names as $table_name ) {
			        ...
			        $query_result  = $wpdb->query( $sql );// phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching

			        // Optional: fire a hook for extensibility
			        /**
			         * Fires after a poll-related table has been dropped.
			         *
			         * @param string $table_name   The table that was dropped.
			         * @param int|false $query_result The result of the drop query.
			         */
			        do_action( 'cbxpoll_table_dropped', $table_name, $query_result );
		        }
	        }
            ...
            //global $post;
            $args = ['posts_per_page' => -1, 'post_type' => 'cbxpoll', 'post_status' => 'any'];
            ...
            wp_safe_redirect(admin_url('edit.php?post_type=cbxpoll&page=cbxpoll-settings#cbxpoll_tools'));
            exit();
        }
    }//end method plugin_full_reset
    ...
    {
        return array_merge([
            'settings' => '<a style="color: #2153cc; font-weight: bold;" target="_blank" href="'.esc_url(admin_url('edit.php?post_type=cbxpoll&page=cbxpoll-settings')).'">'.esc_html__('Settings',
                    'cbxpoll').'</a>'
        ], $links);
    ...
    public function custom_plugin_row_meta($links_array, $plugin_file_name, $plugin_data, $status)
    {
        if ( strpos( $plugin_file_name, CBXPOLL_BASE_NAME ) !== false ) {
            ...
            $links_array[] = '<a target="_blank" style="color:#f44336 !important; font-weight: bold;" href="https://wordpress.org/support/plugin/cbxpoll/" aria-label="' . esc_attr__( 'Free Support', 'cbxpoll' ) . '">' . esc_html__( 'Free Support', 'cbxpoll' ) . '</a>';
            $links_array[] = '<a target="_blank" style="color:#f44336 !important; font-weight: bold;" href="https://wordpress.org/plugins/cbxpoll/#reviews" aria-label="' . esc_attr__( 'Reviews', 'cbxpoll' ) . '">' . esc_html__( 'Reviews', 'cbxpoll' ) . '</a>';
            $links_array[] = '<a target="_blank" style="color:#f44336 !important; font-weight: bold;" href="https://codeboxr.com/doc/cbxpoll-doc/" aria-label="' . esc_attr__( 'Documentation', 'cbxpoll' ) . '">' . esc_html__( 'Documentation', 'cbxpoll' ) . '</a>';


            if ( in_array( 'cbxpollproaddon/cbxpollproaddon.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || defined( 'CBXPOLLPROADDON_PLUGIN_NAME' ) ) {
                //nothing here
            } else {
                $links_array[] = '<a target="_blank" style="color:#f44336 !important; font-weight: bold;" href="https://codeboxr.com/product/cbx-poll-for-wordpress/#downloadarea" aria-label="' . esc_attr__( 'Try Pro Addon', 'cbxpoll' ) . '">' . esc_html__( 'Try Pro Addon', 'cbxpoll' ) . '</a>';
            }
        }
    ...
    public function plugin_activate_upgrade_notices()
    {
        ...
        if (get_transient('cbxpoll_activated_notice')) {
            ...
            echo '<p>'.sprintf(wp_kses(__('Thanks for installing/deactivating <strong>CBX Poll</strong> V%1$s - <a href="%2$s" target="_blank">Codeboxr Team</a>',
                    'cbxpoll'), ['a' => ['href' => [], 'target' => [], 'class' => []]]), esc_attr(CBXPOLL_PLUGIN_VERSION), 'https://codeboxr.com/product/cbx-poll-for-wordpress/').'</p>';
    ...
    public function plugin_compatibility_check()
    {
        ...
        if (in_array('cbxpollproaddon/cbxpollproaddon.php', apply_filters('active_plugins',
            ...
            if (version_compare($pro_plugin_version, '2.0.0', '<')) {
                echo '<div style="border-left:4px solid #d63638;" class="notice notice-error is-dismissible"><p>'.esc_html__('Compatibility issue: CBX Poll V2.0.0 or later needs CBX Poll Pro Addon V2.0.0 or later. Please update CBX Poll Pro Addon to version 2.0.0 or later  - Codeboxr Team',
                        'cbxpoll').'</p></div>';
            }
        } else {
            /* translators: 1: Plugin details url  */
            echo '<div style="border-left:4px solid #2153cc;" class="notice notice-success is-dismissible"><p>'.sprintf(wp_kses(__('<a target="_blank" href="%s">CBX Poll Pro Addon</a> has some extra pro features, try it. - Codeboxr Team',
                    'cbxpoll'), ['a' => ['href' => [], 'target' => [], 'class' => []]]), 'https://codeboxr.com/product/cbx-poll-for-wordpress/').'</p></div>';
        }
    ...
    public function check_pro_addon() {
        cbxpoll_check_and_deactivate_plugin( 'cbxpollproaddon/cbxpollproaddon.php', '2.0.0', 'cbxpolln_proaddon_deactivated' );
    ...
    public function custom_message_after_plugin_row_proaddon($plugin_file, $plugin_data){
        ...
        if(defined('CBXPOLLPROADDON_PLUGIN_NAME')) return;
        ...
        if($pro_addon_version != '' && version_compare( $pro_addon_version, $pro_latest_version, '<' ) ){
            ...
            $custom_message     = wp_kses(sprintf( __( '<strong>Note:</strong> CBX Poll Pro Addon is custom addon plugin. This plugin can not be auto update from dashboard/plugin manager. For manual update please check <a target="_blank" href="%1$s">documentation</a>. <strong style="color: red;">It seems this plugin\'s current version is older than %2$s . To get the latest pro addon features, this plugin needs to upgrade to %2$s or later.</strong>', 'cbxpoll' ), esc_url( $plugin_manual_update ), $pro_latest_version ), ['strong' => ['style' => []],'a' => ['href' => [], 'target' => []]]);
