...
if ( ! function_exists( 'cbxpoll_rgb_from_hex' ) ) {
	 ...
	 * @return array
	 */
	function cbxpoll_rgb_from_hex( $color ) {
...
if ( ! function_exists( 'cbxpoll_hex_darker' ) ) {
	 ...
	 * @return string
	 */
	function cbxpoll_hex_darker( $color, $factor = 30 ) {
		$base  = cbxpoll_rgb_from_hex( $color );
		$color = '#';

		foreach ( $base as $k => $v ) {
			$amount      = $v / 100;
			$amount      = cbxpoll_round( $amount * $factor );
...
if ( ! function_exists( 'cbxpoll_hex_lighter' ) ) {
	 ...
	 * @return string
	 */
	function cbxpoll_hex_lighter( $color, $factor = 30 ) {
		$base  = cbxpoll_rgb_from_hex( $color );
		$color = '#';
...
if ( ! function_exists( 'cbxpoll_hex_is_light' ) ) {
	 ...
	 */
	function cbxpoll_hex_is_light( $color ) {
...
if ( ! function_exists( 'cbxpoll_light_or_dark' ) ) {
	 ...
	 * @return string
	 */
	function cbxpoll_light_or_dark( $color, $dark = '#000000', $light = '#FFFFFF' ) {
		return cbxpoll_hex_is_light( $color ) ? $dark : $light;
	}
}

if ( ! function_exists( 'cbxpoll_format_hex' ) ) {
	 ...
	 * @return string|null
	 */
	function cbxpoll_format_hex( $hex ) {
 ...
 */
function cbxpoll_round( $val, int $precision = 0, int $mode = PHP_ROUND_HALF_UP ): float {
	if ( ! is_numeric( $val ) ) {
		$val = floatval( $val );
	}
 ...
 * @return float
 */
function cbxpoll_array_sum( array $arr ): float {
