...
class PollHelper {
	...
	public static function getVoteCountByStatus( $poll_id = 0 ) {
		...
		$results = $wpdb->get_results( "$sql_select", 'ARRAY_A' );//phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	public static function cbxpoll_single_display( $post_id = 0, $reference = 'shortcode', $result_chart_type = '', $grid = '', $description = '' ) {
		...
		if ( new \DateTime( $poll_start_date ) <= new \DateTime() ) {
			...
			if ( $log_method == 'cookie' ) {
				...
				$poll_is_voted_by_user = $wpdb->get_var( $sql ); //phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
			...
			if ( $is_poll_expired ) { // if poll has expired
				...
				if ( $answers_by_user !== null ) {
					...
					if ( $answers_by_user_html != "" ) {
						...
						$poll_output .= '<p class="cbxpoll-voted-info cbxpoll-voted-info-' . $post_id . '">' . wp_kses( sprintf( __( 'The Poll is out of date. You have already voted for <strong>"%s"</strong>',
					...
					$poll_output .= '<p class="cbxpoll-voted-info cbxpoll-voted-info-' . $post_id . '"> ' . esc_html__( 'The Poll is out of date. You have not voted.',
							'cbxpoll' ) . '</p>';
				}
				...
				if ( ( sizeof( $allowed_user_group ) ) < 1 ) {
					...
					if ( $poll_show_result_before_expire == 1 ) {
						...
						$poll_output .= '<p class="cbxpoll-voted-info cbxpoll-voted-info-' . $post_id . '"> ' . esc_html__( 'You are not allowed to vote.',
								'cbxpoll' ) . '</p>';
					...
					if ( $poll_is_voted_by_user ) {
						...
						$answers_by_user = $wpdb->get_var( $sql );//phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
						...
						foreach ( $poll_answers as $index => $answer ) {
							...
							$cbxpoll_form_answer_listitem_label_extra = '';

							$poll_form_html .= apply_filters('cbxpoll_form_answer_listitem_label_extra', $cbxpoll_form_answer_listitem_label_extra, $post_id, $index, $answer, $poll_answers_extra_single);
	...
	public static function getAllVotesByUser(
		...
		return $wpdb->get_results( "$sql_select  WHERE  $where_sql $sortingOrder  $limit_sql", 'ARRAY_A' );//phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
	...
	public static function getVoteCount( $poll_id = 0, $status = 'all', $vote_id = 0 ) {
		...
		return $wpdb->get_var( "$sql_select  WHERE  $where_sql" );//phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
